...
"""
WebGrid: web-аналог TkGrid3 для отображения таблицы в HTML - Socket
Задача класса:
...
- отображать таблицу в браузере на странице HTML с realtime-обновлением (SSE).
  ...
  <script>
    ...
    async function fetchInitialState() {
      try {
        ...
        // Игнорируем: EventSource продолжит попытки подключения.
      }
    }
    ...
    startRealtime();
    setInterval(() => {
      // Если поток долго молчит, подстрахуемся обычным snapshot-запросом.
                    ...
                    try:
                        ...
                        while not grid._stop_event.is_set():
                            ...
                            if version != last_sent_version and (now - last_sent_ts) >= min_interval:
                                ...
                                # Keep-alive ping, чтобы прокси/браузер не считали соединение idle.
